...
#define VER_SCN_HDMI_1080P_PERCENTAGE_DEVIATION 20
                          ...
                          int dstfb_id,int dstfb_bufno)
{
	...
	{
		blit_para.flag                 = G2D_BLT_NONE;//G2D_BLT_ROTATE90;
	}
	else
	{
		blit_para.flag                 = G2D_BLT_NONE;
...
static int display_setmode(struct display_device_t *dev,int mode,struct display_modepara_t *para)
{
    ...
    if(mode == DISPLAY_MODE_DUALSAME)
    {
		...
		if(ctx->width[0] < ctx->height[0])
		{
			...
			{
				ctx->area_percent[1] = ctx->height[0] * 100 / ctx->height[1] - VER_SCN_HDMI_1080P_PERCENTAGE_DEVIATION;
			}
			else
			{
				if((ctx->height[1] == 1080) && (ctx->height[0] > 1024))
				{
					ctx->area_percent[1] = ctx->height[1] * 100 / ctx->height[0] - 2 * VER_SCN_HDMI_1080P_PERCENTAGE_DEVIATION;
				}
				else
				{
					ctx->area_percent[1] = ctx->height[1] * 100 / ctx->height[0];
				}
			}
			scn0WvH = ctx->width[0] * 100 / ctx->height[0];
		}
		...
		if(ctx->width[0] < ctx->height[0])
		{
			...
			ctx->valid_width[1] = (ctx->valid_height[1] * 100) / scn0WvH;
		}
		else
		{
		...
		if(ctx->width[0] < ctx->height[0])
		{
			scn_win.y = (ctx->height[1]*4/100)/2;
	        scn_win.height = ctx->height[1]*96/100;
		}
		else
		{
			scn_win.y = (ctx->height[1] - ctx->valid_height[1])/2;
